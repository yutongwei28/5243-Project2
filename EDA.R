...
# --- A) Data Upload ---
  ...
  observeEvent(input$feature_engineer, {
    req(data())
    df <- data()
    new_expr <- input$new_feature
    if (new_expr == "") {
      showNotification("No feature expression provided.", type = "warning")
      return()
    }
  
    # Parse user expression "new_col = expression"
    parts <- strsplit(new_expr, "=")[[1]]
    if (length(parts) != 2) {
      showNotification("Expression must be in the form 'new_col = expression'", type = "error")
      return()
    }
    new_col_name <- trimws(parts[1])
    expr_rhs <- parts[2]
    
    # Attempt to add the new feature
    tryCatch({
      df <- df %>% mutate(!!new_col_name := eval(parse(text = expr_rhs)))
      data(df)
      showNotification("New feature added successfully!", type = "message")
    }, error = function(e) {
      showNotification(paste("Error in feature engineering:", e$message), type = "error")
    })
  })
