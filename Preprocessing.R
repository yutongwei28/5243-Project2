...
server <- function(input, output, session) {
  ...
  # dynamically generate column selection for analysis
  ...
  output$numeric_filter <- renderUI({
    ...
    sliderInput(
      ...
      min = min(df[[numeric_cols[1]]]),
      max = max(df[[numeric_cols[1]]]),
      value = c(min(df[[numeric_cols[1]]]), max(df[[numeric_cols[1]]]))
    )
  })
  ...
  output$eda_plot <- renderPlotly({
    ...
    if (input$plot_type == "Scatter Plot") {
      p <- ggplot(df, aes_string(x = colnames(df)[1], y = input$selected_var)) +
        geom_point() +
        labs(x = colnames(df)[1], y = input$selected_var)
    } else if (input$plot_type == "Histogram") {
      p <- ggplot(df, aes_string(x = input$selected_var)) +
        geom_histogram(
          bins=30,
          fill = "cornflowerblue",
          color = "black"
      ...
      dat <- data.frame(value=df[[input$selected_var]])
      p <- ggplot(dat, aes(x="", y=value)) +
        geom_boxplot(fill = "cornflowerblue", color = "black") +
        labs(x = input$selected_var, y = "Value")
    }
